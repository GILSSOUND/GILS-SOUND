...
app.post('/api/business', async (req, res) => {
    const { name, description, reqGenre, reqMood } = req.body;
    
    if (!name || !description) {
        return res.status(400).json({ error: "상호명과 회사 소개는 필수 입력 사항입니다." });
    }
                ...
                parts: [
                    {
                        text: `
너는 천재적인 광고 음악 작사가야.
구글 검색(Google Search Tool)을 이용해서 '${name}'에 대한 실제 매장이나 브랜드 정보를 찾아봐.
...
# 노래 제목
(여기에 ${name}이 들어간 짧은 제목 작성)

# 작곡 프롬프트
${reqMood} mood, ${reqGenre} style music, catchy, commercial song, promo jingle
...
app.post('/api/music', async (req, res) => {
    try {
        ...
        const imagePrompt = `Create a beautiful, highly aesthetic background photography or abstract digital art representing the musical genre of: ${genreLabel || style || 'Pop Music'}. VERY IMPORTANT: PURELY VISUAL. ABSOLUTELY NO TEXT, NO LETTERS, NO TYPOGRAPHY. Just draw a beautiful empty background scene without any words.`;
